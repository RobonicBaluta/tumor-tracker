    ...
    <div v-if="show" class="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm"
      ...
      <div class="bg-[#0d1b2a] border border-yellow-500/40 rounded-2xl shadow-2xl shadow-yellow-900/30 w-full max-w-md">
        ...
        <div v-if="mode === 'create'" class="p-5 space-y-4">
          <p class="text-white/60 text-xs font-mono">Apuesta sobre el resultado de esta partida. El otro lado lo toma quien acepte tu link.</p>
          ...
          <div>
            <p class="text-white/40 text-[10px] font-mono tracking-widest mb-2">CANTIDAD (MAX {{ balance }} TC)</p>
          ...
          <button @click="onCreate" :disabled="creating || !side || amount <= 0 || amount > balance"
            ...
            {{ creating ? 'Creando...' : `Apostar ${amount} TC por ${side === 'blue' ? '🔵' : '🔴'}` }}
          </button>
        </div>
        ...
        <div v-else-if="mode === 'created'" class="p-5 space-y-4">
          <div class="bg-green-950/40 border border-green-500/40 rounded-xl p-4 text-center">
            <p class="text-green-300 text-sm font-mono font-bold">✓ Apuesta creada</p>
            <p class="text-white/60 text-xs font-mono mt-1">{{ createdBet?.amount }} TC escrowadas. Comparte el link para que alguien acepte.</p>
          </div>

          <div>
            <p class="text-white/40 text-[10px] font-mono tracking-widest mb-2">CÓDIGO</p>
        ...
        <div v-else-if="mode === 'accept' && betToAccept" class="p-5 space-y-4">
          <div class="bg-black/30 border border-white/10 rounded-xl p-4 text-center">
            <p class="text-white/40 text-[10px] font-mono tracking-widest">{{ betToAccept.creator?.username }} APOSTÓ POR</p>
          ...
          <p class="text-white/70 text-sm font-mono text-center">
            Si aceptas, apuestas <span class="text-yellow-300 font-bold">{{ betToAccept.amount }} TC</span> a que gana
            <span :class="betToAccept.creator_side === 'blue' ? 'text-red-300' : 'text-blue-300'" class="font-bold">
              {{ betToAccept.creator_side === 'blue' ? '🔴 ROJO' : '🔵 AZUL' }}
            </span>.
          </p>
          ...
          <div class="flex gap-2">
            ...
            <button @click="onAccept" :disabled="accepting || (betToAccept.amount > balance)"
              ...
              {{ accepting ? 'Aceptando...' : (betToAccept.amount > balance ? 'Saldo insuficiente' : 'Aceptar') }}
            </button>
          </div>
        </div>
      </div>
...
const title = computed(() => {
  if (props.mode === 'create') return 'Crear apuesta'
  if (props.mode === 'created') return '¡Apuesta creada!'
  if (props.mode === 'accept') return 'Aceptar apuesta'
  return ''
})
