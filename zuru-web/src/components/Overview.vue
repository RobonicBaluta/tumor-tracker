<template>
  <div ref="scrollContainerRef" class="flex-1 flex flex-col overflow-y-auto relative"
    ...
    <!-- Live game modal (lazy-loaded via defineAsyncComponent — chunk
         se baja en el primer v-if=true para no inflar el initial bundle).
         Toda la lógica de polling/resolve/state vive en Overview, este
         componente sólo renderiza props y emite events. -->
    <LiveGameModal
      :show="showLiveGame"
      :live-game="liveGame"
      ...
      :prediction-stats="predictionStats"
      :live-prediction="livePrediction"
      :has-prediction="hasPrediction"
      :is-arena="isArena"
      :is-live-queue-bettable="isLiveQueueBettable"
      :blacklisted-in-team="(blacklistedInTeam as any[])"
      :arena-sorted-players="(arenaSortedPlayers as any[])"
      :lane-matchups="(laneMatchups as any[])"
      :champion-blacklist="championBlacklist"
      :champ-data="champData"
      :ddragon-version="ddragonVersion"
      :resolving="resolving"
      :resolve-result="resolveResult"
      :existing-player-bet="(existingPlayerBet as any)"
      :can-bet-player="(canBetPlayer as any)"
      :profile-url="profileUrl"
      @close="closeLiveGame"
      @open-create-bet="openCreateBet"
      @open-player-bet="(openPlayerBet as any)"
      @toggle-blacklist="toggleBlacklist"
      @resolve-live-prediction="resolveLivePrediction"
      @retry-refresh="searchLiveGame(true)" />
...
import {
  ...
  ROLE_ORDER, TIER_COLORS,
  tumorColor, tumorLabel,
  championIconUrl, championLoadingUrl, championIconFallback,
} from '../composables/overviewConstants'
// RUNE_STYLES, SUMMONER_SPELLS, ROLE_LABEL movidos a LiveGameModal.vue
// (extracción #16). spellIconUrl helper también — usaba SUMMONER_SPELLS y
// solo se usaba dentro del modal extraído.
...
// fg/bg = colores hex. quietZone = módulos de margen blanco (recomendado 4).
function _paintQR(
  ctx: CanvasRenderingContext2D,
  qr: { getModuleCount(): number; isDark(r: number, c: number): boolean },
  x: number, y: number, size: number,
  fg = '#000', bg = '#fff', quietZone = 4,
) {
  const moduleCount = qr.getModuleCount()
  const total = moduleCount + quietZone * 2
  const cell = size / total
  // Fondo blanco completo (incluye quiet zone)
  ctx.fillStyle = bg
  ...
  for (let r = 0; r < moduleCount; r++) {
    for (let c = 0; c < moduleCount; c++) {
      if (qr.isDark(r, c)) {
        const px = x + (c + quietZone) * cell
        const py = y + (r + quietZone) * cell
...
// Texto narrativo según el tumor avg
function _tumorLabel(t: number): string {
  if (t < 20) return 'CRACKED'
  if (t < 40) return 'LIMPIO'
  if (t < 55) return 'NORMAL'
  if (t < 65) return 'SUS'
  if (t < 80) return 'INFLAMADO'
  return 'TUMOR MALIGNO'
}
function _tumorColor(t: number): string {
  if (t < 20) return '#22c55e'      // green-500
  if (t < 40) return '#84cc16'      // lime-500
  if (t < 55) return '#eab308'      // yellow-500
  if (t < 65) return '#f97316'      // orange-500
  if (t < 80) return '#ef4444'      // red-500
  return '#b91c1c'                  // red-700
}

// Texto wrapping helper para no salirse del card
function _fitText(ctx: CanvasRenderingContext2D, text: string, maxWidth: number): string {
...
// spellIconUrl movido a LiveGameModal.vue (era solo usado dentro de él).
